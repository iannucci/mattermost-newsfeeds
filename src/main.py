...
def load_sources(cfg, logger, seen, notifier):
    general=cfg['general']; out=[]
    for s in cfg.get('sources',[]):
        if not s.get('enabled',True): continue
        mod=importlib.import_module(s['module']); cls=getattr(mod, s['class'])
        inst=cls(s.get('name', s['class']), s, general, seen, logger, notifier)
        inst.schedule_next(); out.append(inst)
...
def find_config_path(cli_path: str):
    ...
    if os.path.exists(cwd_cfg): return cwd_cfg
    return cli_path
...
def scheduler_loop(cfg, logger):
    ...
    sleep_min=int(cfg['general'].get('sleep_min',1)); sleep_max=int(cfg['general'].get('sleep_max',5))
    ...
    while True:
        ...
        for s in sources:
            if s.due():
                try: s.poll(now)
                except Exception as e: logger.exception(f"Error polling {s.name}: {e}")
                s.schedule_next(); ran=True
