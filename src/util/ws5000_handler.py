...
class Handler:
    ...
    def start(self) -> None:
        if self.mode == "udp":
            self._thread = threading.Thread(
                target=self._udp_loop, name="WS5000-UDP", daemon=True
            )
        else:
            self._thread = threading.Thread(
                target=self._http_loop, name="WS5000-HTTP", daemon=True
            )
        self._thread.start()
    ...
    def _http_loop(self) -> None:
            ...
            def _fields(self):
                ...
                if not fields:
                    ...
                    if "=" in tail:
                        ...
                        fields = {
                            k: v[-1]
                            for k, v in parse_qs(cand, keep_blank_values=True).items()
                        }
                if self.command == "POST":
                    ctype = (
                        (self.headers.get("Content-Type", "").split(";", 1)[0] or "")
                        .strip()
                        .lower()
                    )
                    if ctype in ("application/x-www-form-urlencoded", "text/plain"):
                        ...
                        pq = parse_qs(
                            body.decode("utf-8", "ignore"), keep_blank_values=True
                        )
        ...
        try:
            self.logger.debug(
                f"[ws5000_handler:http] Listening on {host}:{port}",
                file=sys.stderr,
                flush=True,
            )
            ...
            self.logger.debug(
                f"[ws5000_handler:http] server error: {e}", file=sys.stderr, flush=True
            )
        finally:
            try:
                server.server_close()
    ...
    def _udp_loop(self) -> None:
        ...
        self.logger.debug(
            f"[ws5000_handler:udp] Capture iface={iface or '(auto)'} port={port}",
            file=sys.stderr,
            flush=True,
        )
        cap = WS5000BroadcastCapture(
            dest_port=port, iface=iface, callback=on_packet, debug=True
        )
        cap.run_blocking()
