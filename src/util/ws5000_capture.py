...
class WS5000BroadcastCapture:
    ...
    def run_blocking(self) -> None:
        print(f"[ws5000_capture] sniff start iface={self.iface or '(auto)'} BPF='{self.bpf}' filter_dport={self.dest_port}",
              file=sys.stderr, flush=True)
        try:
            sniff(filter=self.bpf, iface=self.iface, store=False, prn=self._on_packet)
        except Exception as e:
            print(f"[ws5000_capture] ERROR starting sniff: {e}", file=sys.stderr, flush=True)
    ...
    def _on_packet(self, pkt) -> None:
        ...
        if self.debug:
            print(f"[ws5000_capture] pkt {meta['src_ip']}:{meta['src_port']} -> {meta['dst_ip']}:{meta['dst_port']} payload={len(payload)}B",
                  file=sys.stderr, flush=True)
        if self.callback:
            try:
                self.callback(payload, meta)
            except Exception as e:
                print(f"[ws5000_capture] callback error: {e}", file=sys.stderr, flush=True)
